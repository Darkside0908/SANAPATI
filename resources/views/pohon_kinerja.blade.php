    ...
    function treeApp() {
        return {
            ...
            applyFilters() {
                ...
                const filtered = this.filterTree(JSON.parse(JSON.stringify(this.treeData)), this.selectedUnit);
                this.renderTree(filtered, document.getElementById('treeRoot'));
                
                // If we have results, expand all to show where they are
                if (filtered.length > 0) {
                    this.expandAllMatching(filtered);
                }
            },

            filterTree(nodes, unitCode) {
                return nodes.filter(node => {
                    // Check if this node matches
                    const matchesUnit = (node.units && node.units.some(u => u.code === unitCode)) || 
                                      (node.indicators && node.indicators.some(i => i.unit_owner === unitCode));
                    
                    // Recursively filter children
                    if (node.children && node.children.length > 0) {
                        node.children = this.filterTree(node.children, unitCode);
                    }
                    
                    // Keep node if it matches or has matching children
                    return matchesUnit || (node.children && node.children.length > 0);
                });
            },

            expandAllMatching(nodes) {
                const traverse = (list) => {
                    list.forEach(node => {
            ...
            async performSearch() {
                ...
                this.loading = true;
                try {
                    let url = `/api/pohon-kinerja/search?q=${this.searchQuery}`;
                    if (this.selectedUnit) {
                        url += `&unit=${this.selectedUnit}`;
                    }
